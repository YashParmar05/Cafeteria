...
if(!isset($_SERVER['HTTP_REFERER'])){
    ...
    // header("location: http://localhost/cafeteria");
...
if(!isset($_SESSION['user_id'])){
	...
	if(isset($_POST['submit'])){
		if(empty($_POST['first_name']) or empty($_POST['last_name']) or empty($_POST['state']) or empty($_POST['street_address']) or empty($_POST['town']) or empty($_POST['zip_code']) or empty($_POST['phone']) or empty($_POST['email']) ){
			...
			$place_orders = $conn->prepare("INSERT INTO orders (first_name, last_name, state, street_address, town, zip_code, phone, user_id, status, total_price) VALUES (:first_name, :last_name, :state, :street_address, :town, :zip_code, :phone, :user_id, :status, :total_price)");

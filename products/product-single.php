...
<?php
	...
	if(isset($_GET['id'])){
		...
		//data fro a single product
		$product = $conn->query("SELECT * FROM products WHERE id='$id'");
		$product->execute();
		$singelProduct = $product->fetch(PDO::FETCH_OBJ);

		//data for a related products
		$relatedProducts = $conn->query("SELECT * FROM products WHERE type='$singelProduct->type' AND id != '$singelProduct->id'");
    	...
    	<div class="container">
    		<div class="row">
    			<div class="col-lg-6 mb-5 ftco-animate">
    				<a href="images/menu-2.jpg" class="image-popup"><img src="<?php echo APPURL; ?>/images/<?php echo $singelProduct->image; ?>" class="img-fluid" alt="Item image"></a>
    			</div>
    			<div class="col-lg-6 product-details pl-md-5 ftco-animate">
    				<h3><?php echo $singelProduct->name; ?></h3>
    				<p class="price"><span><?php echo $singelProduct->price; ?></span></p>
    				<p><?php echo $singelProduct->description; ?>
						</p>
					...
					<form method="POST" action="product-single.php?id=<?php echo $id; ?>">
						...
						<div class="input-group col-md-6 d-flex mb-3">
							...
							<input type="text" id="quantity" name="quantity" class="form-control input-number" value="1" min="1" max="100" step="1">
					...
					<?php if(isset($_SESSION['user_id'])) :  ?>
						<?php if($rowCount > 0) : ?>
							<button  name="submit" type="submit" class="btn btn-primaryy py-3 px-5" disabled>Added to Cart</button>
						<?php else : ?>
							<button  name="submit" type="submit" class="btn btn-primaryy py-3 px-5">Add to Cart</button>
						<?php endif; ?>
					<?php else :  ?>
						<p><b>login to add product to cart</b></p>
					<?php endif ; ?>
